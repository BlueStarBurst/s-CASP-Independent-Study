...
action(pick_log, pick_up_entity, GUID) :- item_on_screen(log, GUID).
action(chop_tree, chop_tree, GUID) :- item_on_screen(X, GUID), choppable(GUID), equipment(axe).
...
?- action(pick_anything , pick_entity , 106316).